...
| `coincurve` | The Nostr channel's NIP-44 encryption needs secp256k1 ECDH and BIP340 Schnorr signing (`channels/nostr/nip44.py`, `nip01.py`). This is exactly the elliptic-curve arithmetic this project does not hand-roll — unlike the padding, HKDF, ChaCha20, Poly1305 and bech32 around it, which stayed stdlib and are checked against RFC/NIP test vectors in `tests/`. | The service's Nostr private key, held in memory only for the duration of a sign/ECDH call. Never the destination npub's private key — that is never in this process. |
